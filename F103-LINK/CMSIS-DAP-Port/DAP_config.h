...
#define DAP_JTAG_DEV_CNT        0U              ///< Maximum number of JTAG devices on scan chain.
...
__STATIC_FORCEINLINE uint32_t PIN_SWCLK_TCK_IN  (void) {
  return HAL_GPIO_ReadPin(SWCLK_GPIO_Port, SWCLK_Pin) ? 1 : 0;
}
...
__STATIC_FORCEINLINE uint32_t PIN_SWDIO_TMS_IN  (void) {
  return HAL_GPIO_ReadPin(SWDIO_GPIO_Port, SWDIO_Pin) ? 1 : 0;
}
...
__STATIC_FORCEINLINE uint32_t PIN_nRESET_IN  (void) {
  return HAL_GPIO_ReadPin(nRESET_GPIO_Port, nRESET_Pin) ? 1 : 0;
}
...
__STATIC_FORCEINLINE void     PIN_nRESET_OUT (uint32_t bit) {
  HAL_GPIO_WritePin(nRESET_GPIO_Port, nRESET_Pin, (bit & 1) ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
*/
__STATIC_INLINE void LED_CONNECTED_OUT (uint32_t bit) { HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, (bit == 0U) ? GPIO_PIN_SET : GPIO_PIN_RESET); }
...
*/
__STATIC_INLINE void LED_RUNNING_OUT (uint32_t bit) { HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, (bit == 0U) ? GPIO_PIN_SET : GPIO_PIN_RESET); }
